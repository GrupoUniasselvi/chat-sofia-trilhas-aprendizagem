...
export const Answer = ({
    ...
    const updateFeedbackList = (ev?: FormEvent<HTMLElement | HTMLInputElement>) => {
        if (answer.message_id == undefined) return;
        let feedback = (ev?.target as HTMLInputElement)?.value;
    ...
    const onSubmitNegativeFeedback = async () => {
        if (answer.message_id == undefined) return;
        await historyMessageFeedback(answer.message_id, negativeFeedback);
        resetFeedbackDialog();
    }
    ...
    return (
        <>
            ...
            >
                <Stack tokens={{childrenGap: 4}}>
                    <div>Seu feedback melhorará esta experiência.</div>
                    
                    <TextField
                        placeholder={'Deixe seu feedback aqui...'}
                        multiline
                        resizable={false}
                        onKeyUp={updateFeedbackList}
                    />
                    
                    <div>Pressionando enviar, seu feedback ficará visível para o proprietário do aplicativo.</div>
                    
                    <DefaultButton onClick={onSubmitNegativeFeedback}>Enviar</DefaultButton>
                </Stack>
